...
#500小时词库
corpus=$1
#500小时语音 data=data/500hour
data=$2


if [ ! -d $corpus/train ]; then
  echo "Error: $0 requires complete corpus"
  exit 1;
fi


echo "**** Creating 500hourdata data folder ****"
if [ ! -d "$data/train" ] || [ ! -d "$data/tmp" ]; then
  echo "$0: no such directory 'train' or 'tmp' in $data, make it."
...
echo "$0: 500hour data preparation succeeded"
exit 0;
