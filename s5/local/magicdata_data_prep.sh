...
#$1=$/mnt/data/openslr/magicdata
corpus=$1
#$2=data/magicdata
data=$2
#-d filename  如果 filename为目录，则为真  [ -d /tmp/mydir ]
#magic数据集在stage0已经处理好了train,dev,test
...
tmp_dir=$data/tmp
#find 搜索目录 -name 目标名字; -iname不用区分大小写
#把所有.wav的名字写进flist
...
for x in train dev test; do
  grep -i "/$x/" $tmp_dir/wav.flist > $data/$x/wav.flist || exit 1;#grep -i 忽略大小写
